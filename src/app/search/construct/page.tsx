...
export default async function ConstructPage(
  props: {
    searchParams: Promise<{ c?: string; w?: string; ord?: string; sv?: string; books?: string }>
  }
) {
  ...
  const translations = await loadContent(await getServerLocale(), 'constructPresets')
  ...
  const trackCorpus = await getServerTrack() === 'hebrew' ? 'MT' : 'GNT'
