...
export default function PublicExegesisPage({ searchParams }: { searchParams: { tab?: string; open?: string } }) {
  ...
  return (
    <main className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden print:h-auto print:overflow-visible w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
      <ExegesisTabs isAuthenticated={isAuthenticated} initialTab={searchParams?.tab} initialOpen={searchParams?.open} />
    </main>
  )
}
