...
function Axis({ label, hint, display, caption, value, range, low, high, marks }: {
  ...
  return (
    <div>
      ...
      {/* The ticks say what they are, right under themselves — a tick a reader has to hover
          to identify is a tick most readers never identify. The two labels grow AWAY from
          each other (the left one ends at its tick, the right one starts at its): centred
          labels collide on a narrow column, and a collision was measured, not imagined. */}
      <div className="relative h-3.5">
        {shown
          .slice()
          .sort((a, b) => pos(a.at) - pos(b.at))
          .map((m, i, all) => {
            const p = pos(m.at) * 100
            const anchor = all.length < 2 ? { left: `${p}%`, transform: 'translateX(-50%)' }
              : i === 0 ? { right: `${100 - p}%`, paddingRight: 3 }
              : { left: `${p}%`, paddingLeft: 3 }
            return (
              <span key={m.label}
                className="absolute whitespace-nowrap text-[9px] leading-4 text-gray-400"
                style={anchor}
              >
                {m.label}
              </span>
