...
export function ExegesisWorkspace() {
  // ── Passage state ──
  ...
  useEffect(() => {
    fetch('/api/reader?corpus=GNT')
      ...
      .then(d => {
        setBooks(d.books ?? [])
        if (d.books?.length) setSelectedBook(d.books[0])
      })
      .catch(() => {})
  }, [])
  ...
  async function saveSession() {
    if (!selectedBook || loadedVerses.length === 0) return
    setIsSaving(true)
    setSaveStatus('idle')
    try {
      const payload = {
        title: sessionTitle || `${selectedBook.name} ${chapter}:${verseStart}${verseEnd !== verseStart ? `–${verseEnd}` : ''}`,
      ...
      if (sessionId) {
        await fetch(`/api/exegesis/${sessionId}`, {
          ...
          body: JSON.stringify({ annotations, title: payload.title }),
        })
      } else {
