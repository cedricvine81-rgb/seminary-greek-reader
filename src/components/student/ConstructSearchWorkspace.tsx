  ...
  if (isLoading) {
    return (
      <p className="py-8 text-center text-sm text-gray-400">
        <Loader2 size={16} className="inline animate-spin" /> {t('cs.loadingFindList')}
      </p>
    )
  }
  if (!data) return <p className="py-8 text-sm italic text-gray-400">{t('cs.couldNotLoad')}</p>
...
function Workspace({ data, assignmentId, previewMode, onSaved }: {
  ...
  async function save(submit: boolean) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error ?? t('cs.saveFailed'))
      setSaved(true)
      setTimeout(() => setSaved(false), 2500)
      onSaved()
    } catch (err) {
      setError(err instanceof Error ? err.message : t('cs.saveFailed'))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-brand-200 bg-brand-50 p-5 space-y-3">
        ...
        {search ? (
          <>
            ...
            <p className="text-xs text-brand-600">{t('cs.inCorpus', { corpus: constructCorpusLabel(search.query) })}</p>
          </>
        ) : (
          <p className="text-sm text-brand-800">{t('cs.noSearchAttached')}</p>
        )}
        {searchHref && (
          <Link href={searchHref} target="_blank" className="btn btn-primary inline-flex w-fit items-center gap-1.5 px-4 py-2">
            {t('cs.runTheSearch')} <ExternalLink size={14} />
          </Link>
        )}
      ...
      {submitted && (
        <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
          <span className="font-semibold">{t('cs.handedIn')}</span> {t('cs.lockedNote')}
        </div>
      )}
        ...
        <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="text-sm font-semibold text-gray-800">
            ...
            <span className={`ml-2 text-xs font-medium ${enough ? 'text-green-700' : 'text-gray-400'}`}>
              {t('cs.nOfMComplete', { done, total: config.requiredCount })}
            </span>
          </h2>
          <p className="text-xs text-gray-400">{t('cs.rowCountsWhen')}</p>
        </div>
            ...
            <div key={i} className="rounded-xl border border-gray-200 bg-surface p-3">
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                <input
                  ...
                  disabled={locked}
                  placeholder={t('cs.refPlaceholder')}
                ...
                <input
                  ...
                  disabled={locked}
                  placeholder={t('cs.greekPlaceholder')}
              ...
              {config.askTranslation && (
                <input
                  ...
                  disabled={locked}
                  placeholder={t('cs.translationPlaceholder')}
              ...
              {config.askComment && (
                <textarea
                  ...
                  disabled={locked}
                  rows={2}
                  placeholder={t('cs.commentPlaceholder')}
                  className="input mt-2 text-sm"
                />
              )}
            </div>
      ...
      {/* ── Conclusions ── */}
      <div>
        <label className="mb-1 block text-sm font-semibold text-gray-800">{t('cs.whatDidYouNotice')}</label>
        <textarea
          value={notes}
      ...
      {!locked && (
        <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          <Button
            onClick={() => {
              if (!enough && !window.confirm(t('cs.confirmIncomplete', { done, total: config.requiredCount }))) return
              if (!window.confirm(t('cs.confirmHandIn'))) return
              save(true)
            }}
            loading={saving}
          >
