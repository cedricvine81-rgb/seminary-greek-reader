    ...
    return (
      <Card>
        <p className="text-sm text-gray-400 py-6 text-center">
          <Loader2 size={16} className="inline animate-spin" /> {t('cn.loading')}
        </p>
      </Card>
    )
  }
  if (!data) return <Card><p className="text-sm text-gray-400 italic py-6">{t('cn.loadFailed')}</p></Card>
  if (data.rows.length === 0) return <Card><p className="text-sm text-gray-400 italic py-6">{t('cn.noStudents')}</p></Card>
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        <CardTitle>{t('cn.title', { name: data.assignment.notesFolderName || data.assignment.title })}</CardTitle>
        <span className="text-sm text-gray-500">
          {t('cn.submittedCount')}: <span className="font-semibold text-gray-800">{submitted} / {data.rows.length}</span>
        </span>
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50">
        <button onClick={onToggle} className="shrink-0 text-gray-400 hover:text-gray-700" title={t(expanded ? 'cn.collapse' : 'cn.expand')}>
        ...
        <div className="shrink-0 text-center w-24">
          {row.submittedAt
            ? <span className="text-xs font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded-md">{t('cn.statusSubmitted')}</span>
            : <span className="text-xs font-medium text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md">{t('cn.statusNotSubmitted')}</span>}
          <p className="mt-1 text-[11px] text-gray-400">{t('cn.noteCount', { count: row.noteCount, n: row.noteCount })}</p>
        </div>
      ...
      {expanded && (
        <div className="px-3 pb-4 pt-1 bg-gray-50/60 space-y-3">
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('gr.feedbackToStudent')}</label>
            <textarea
              value={gradeNote}
                  ...
                  <div key={i} className="rounded-lg border border-gray-200 bg-surface p-3">
                    <div className="flex items-center gap-2 mb-1.5">
                      {n.ref
                        ...
                        : <span className="text-xs font-semibold text-gray-700">{n.title || t('cn.courseNote')}</span>}
                      {!n.ref && (
                        <span className="shrink-0 rounded-full bg-gray-100 text-gray-500 text-[10px] px-1.5 py-0.5">{t('cn.courseNote')}</span>
                      )}
                    </div>
                    ...
                    {isHtmlEmpty(html)
                      ? <p className="text-xs text-gray-400 italic">{t('cn.emptyNote')}</p>
                      : <div
