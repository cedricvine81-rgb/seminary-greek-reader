...
export function BookPicker({ groups, selected, onToggle, onToggleGroup, onClear }: {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white">
      ...
      <div className="p-2 max-h-[36vh] overflow-y-auto">
        {groups.map(g => {
          ...
          return (
            <div key={g.heading} className="mb-3 last:mb-0">
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400">{g.heading}</p>
                <button type="button" onClick={() => onToggleGroup(ids, !allSel)}
                  className="text-[11px] text-brand-600 hover:underline">{allSel ? 'Clear' : 'Select all'}</button>
              </div>
                ...
                {g.books.map(b => {
                  ...
                  return (
                    <button key={b.osisId} type="button" onClick={() => onToggle(b.osisId)}
                      title={b.name}
                      className={`rounded-lg border px-1 py-1.5 text-[12px] leading-tight font-medium transition-colors ${
