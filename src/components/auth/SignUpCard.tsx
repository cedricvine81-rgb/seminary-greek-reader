      ...
      <div className="w-full max-w-lg">
        <div className="bg-surface rounded-xl shadow-sm border border-parchment-200 p-8">
          <div className="text-center mb-6">
            ...
            <h1 className="text-xl font-bold text-ink-900">{t(headingKey)}</h1>
            <p className="text-sm text-gray-500 mt-1">{t(subheadingKey)}</p>
          </div>
          <SignUpForm lockedRole={lockedRole} />
          <p className="text-center text-sm text-gray-500 mt-4">
            {t('auth.haveAccount')}{' '}
            <Link href="/auth/sign-in" className="text-brand-600 hover:underline font-medium">{t('auth.signInLink')}</Link>
          </p>
