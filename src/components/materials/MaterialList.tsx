import Link from 'next/link'
import { FileText, ExternalLink } from 'lucide-react'

interface Material {
  id: string
  courseId?: string
  courseName?: string
  title: string
  description?: string | null
  fileUrl?: string | null
  weekNumber?: number | null
  createdAt: string
}

interface MaterialListProps {
  materials: Material[]
  canEdit?: boolean
  groupByCourse?: boolean
}

function renderMaterial(m: Material, canEdit?: boolean) {
  return (
    <div key={m.id} className="flex items-start justify-between p-3 rounded-lg border border-gray-100 bg-white hover:bg-gray-50 transition-colors">
      <div className="flex items-start gap-2">
        <FileText size={16} className="text-brand-600 mt-0.5 shrink-0" />
        <div>
          <p className="text-sm font-medium text-gray-900">{m.title}</p>
          {m.description && <p className="text-xs text-gray-500 mt-0.5">{m.description}</p>}
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        {m.fileUrl && (
          <a href={m.fileUrl} target="_blank" rel="noopener noreferrer" className="text-brand-600 hover:text-brand-800">
            <ExternalLink size={14} />
          </a>
        )}
        {canEdit && (
          <Link href={`/instructor/materials/${m.id}`} className="text-xs text-gray-500 hover:text-gray-800">
            Edit
          </Link>
        )}
      </div>
    </div>
  )
}

function WeekGrouped({ materials, canEdit }: { materials: Material[]; canEdit?: boolean }) {
  const grouped: Record<number, Material[]> = {}
  const noWeek: Material[] = []
  for (const m of materials) {
    if (m.weekNumber != null) {
      grouped[m.weekNumber] = [...(grouped[m.weekNumber] ?? []), m]
    } else {
      noWeek.push(m)
    }
  }

  return (
    <div className="space-y-4">
      {Object.entries(grouped).sort(([a], [b]) => Number(a) - Number(b)).map(([week, items]) => (
        <div key={week}>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Week {week}</h3>
          <div className="space-y-1">{items.map(m => renderMaterial(m, canEdit))}</div>
        </div>
      ))}
      {noWeek.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">General</h3>
          <div className="space-y-1">{noWeek.map(m => renderMaterial(m, canEdit))}</div>
        </div>
      )}
    </div>
  )
}

export function MaterialList({ materials, canEdit, groupByCourse }: MaterialListProps) {
  if (materials.length === 0) {
    return <p className="text-sm text-gray-400 italic py-4">No materials yet.</p>
  }

  if (!groupByCourse) {
    return <WeekGrouped materials={materials} canEdit={canEdit} />
  }

  // Group by course first, then by week within each course
  const courseOrder: string[] = []
  const byCourse: Record<string, { name: string; items: Material[] }> = {}
  for (const m of materials) {
    const key = m.courseId ?? 'unknown'
    if (!byCourse[key]) {
      byCourse[key] = { name: m.courseName ?? 'Unknown Course', items: [] }
      courseOrder.push(key)
    }
    byCourse[key].items.push(m)
  }

  return (
    <div className="space-y-8">
      {courseOrder.map(courseId => (
        <div key={courseId}>
          <h2 className="text-sm font-semibold text-gray-700 mb-3 pb-2 border-b border-gray-100">
            {byCourse[courseId].name}
          </h2>
          <WeekGrouped materials={byCourse[courseId].items} canEdit={canEdit} />
        </div>
      ))}
    </div>
  )
}
