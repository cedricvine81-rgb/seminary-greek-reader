...
def align_page(window: list[dict], sp: list[str]):
...
def main():
    ...
    # RESUME: pick up after the last completed page, cursor = words already consumed.
    ...
    while cursor < len(mt) and page < start_page + 160:
        ...
        consumed, matched, ops = align_page(window, sp)
