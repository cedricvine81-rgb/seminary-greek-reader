...
export function registerBackgroundItems(): Item[] {
  ...
  for (const sec of BACKGROUND) {
    ...
    sec.groups?.forEach((g, j) => {
      items.push({ key: `regbg.${sec.id}.g${j}.h`, english: g.heading })
      g.paragraphs.forEach((p, i) => items.push({ key: `regbg.${sec.id}.g${j}.p${i}`, english: p }))
      g.items?.forEach((it, k) => items.push({ key: `regbg.${sec.id}.g${j}.i${k}`, english: it }))
    })
  }
  return items
}
